...
CREATE_SECRETS = ("cloudflare-api-token", "clickhouse-password", "pocketbase-superuser-password", "github-token", "lightdash-admin-password", "lightdash-secret", "lightdash-postgres-password", "lightdash-clickhouse-password", "lightdash-r2-access-key-id", "lightdash-r2-secret-access-key")
...
def state_errors(opts: dict) -> list[str]:
    ...
    if opts.get("blue/event") != "run":
        errors.extend(compute.errors(opts))
...
def secret_errors(opts: dict, event: str) -> list[str]:
    ...
    required = RUN_SECRETS if event == "run" else (("cloudflare-api-token",) if event == "delete" else CREATE_SECRETS) + backend
