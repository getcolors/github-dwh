...
from colors_compute import orchestrate, plan_deployment, read_deployment, validate, backend_plan

TOPOLOGY = [{'count': 1}]


def requirements(opts):
    return {'single_host': True, 'network': {'mode': 'none'}, 'backups': {'enabled': True, 'schedule': {'type': 'daily', 'hour': 3}}, 'ipv6': False, 'security': {'egress': 'all', 'private_filter': False, 'ingress': [
        {'id': 'port-' + str(port), 'protocol': 'tcp', 'from_port': port, 'to_port': port, 'sources': ['0.0.0.0/0']}
        for port in (22, 80, 443)]}, 'legacy_state_keys': [opts['profile'] + '/tofu.tfstate']}
...
def attach(opts, result):
    ...
    if result.get('status') == 'destroyed':
        return {**opts, 'blue/exit': 0, 'github-dwh/already-destroyed': True}
